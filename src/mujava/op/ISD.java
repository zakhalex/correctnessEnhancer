...
/**
 * <p>
 * Generate ISI (Super keyword insertion) and
...
public class ISD extends mujava.op.util.Mutator
{
	...
	public ISD(FileEnvironment file_env, ClassDeclaration cdecl, CompilationUnit comp_unit, String className)
	{
	...
	public void visit(ClassDeclaration p) throws ParseTreeException
	{
		...
		if (p.getName().equalsIgnoreCase(className.substring(unQualifier, className.length())))
		{
			nesting = 1;
		}
		else if (nesting == 0)
	...
	public void outputToFile(FieldAccess original, FieldAccess mutant)
	{
		...
		String f_name;
		num++;
		f_name = getSourceName(this, className, getMutantType("ISD"));
	...
	public String getISISourceName(mujava.op.util.Mutator clazz)
	{
		...
		String dir_name = MutationSystem.MUTANT_HOME + File.separator + className + File.separator
				+ getMutantType("ISD") + "/ISI_" + this.num;
