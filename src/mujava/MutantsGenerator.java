...
/**
 * <p>
 * Generate mutants according to selected mutation
...
public abstract class MutantsGenerator
{
	...
	public boolean makeMutants(String originalPath, String mutantPath, String className) throws OpenJavaException
	{
		...
		genMutants(mutantPath, className);
		Debug.print("..done.\n");
	...
	abstract void genMutants(String mutantPath, String className);
	...
	private void arrangeOriginal(String originalPath, String className)
	{
		...
		for (int j = 0; j < cdecls.size(); ++j)
		{
			...
			{
				System.err.println("MutantGenerator fails to create " + outfile);
			}
	...
	public static boolean generateParseTree(File f, CompilationUnit[] comp_unit, FileEnvironment[] file_env, String className)
	{
		try
		{
			...
			for (int j = 0; j < typedecls.size(); ++j)
			{
				...
				int unQualifier = className.lastIndexOf('.') + 1;
				if (class_decl.getName().equalsIgnoreCase(className.substring(unQualifier, className.length())))
				{
	...
	private void compileOriginal(String originalPath, String className)
	{
		...
		String[] pars = { "-classpath", MutationSystem.CLASS_PATH,
				originalPath + "/" + className.substring(unQualifier, className.length()) + ".java" };
		try
		{
			// result = 0 : SUCCESS, result = 1 : FALSE
			// int result = Main.compile(pars,new PrintWriter(new FileOutputStream("temp")));
