...
public class AllMutantsGenerator extends MutantsGenerator
{
	...
	void genMutants(String mutantPath, String className)
	{
		...
		if (traditionalOp != null && traditionalOp.length > 0)
		{
			...
			genTraditionalMutants(cdecls, className);
		...
		if (classOp != null && classOp.length > 0)
		{
			...
			CodeChangeLog.openLogFile(mutantPath);
			genClassMutants(cdecls, className);
			CodeChangeLog.closeLogFile();
		}
	}

	void genClassMutants(ClassDeclarationList cdecls, String className)
	{
		genClassMutants1(cdecls, className);
		genClassMutants2(cdecls, className);
	}

	void genClassMutants2(ClassDeclarationList cdecls, String className)
	{
		HashSet<String> ignore = new HashSet<String>(Arrays.asList("IHD", "IHI", "IOD", "OMR", "OMD", "JDC"));
	...
	void genClassMutants1(ClassDeclarationList cdecls, String className)
	{
		HashSet<String> ignore = new HashSet<String>(Arrays.asList("IHD", "IHI", "IOD", "OMR", "OMD", "JDC"));
	...
	void genTraditionalMutants(ClassDeclarationList cdecls, String className)
	{
		for (int j = 0; j < cdecls.size(); ++j)
		{
			ClassDeclaration cdecl = cdecls.get(j);
			...
			if (cdecl.getName().equalsIgnoreCase(className.substring(unQualifier, className.length())))
			{
				try
				{
					...
					// mutant_op = new AODS(file_env, cdecl, comp_unit, className);
					// comp_unit.accept(mutant_op);
					// }
					//
